...
def select_device(device="", batch=0, newline=False, verbose=True):
    ...
    original = str(device).lower()
    device = original
    for remove in "cuda:", "none", "(", ")", "[", "]", "'", " ":
        device = device.replace(remove, "")
    cpu = device == "cpu"
    mps = device in {"mps", "mps:0"}
    xpu_req = "xpu" in original
    if cpu or mps:
        os.environ["CUDA_VISIBLE_DEVICES"] = "-1"
    elif device and not xpu_req:
        if device == "cuda":
            device = "0"
        if "," in device:
    ...
    if not cpu and not mps and not xpu_req and torch.cuda.is_available():
        ...
        arg = "mps"
    elif xpu_req and hasattr(torch, "xpu") and torch.xpu.is_available():
        try:
            props = torch.xpu.get_device_properties(0)
            name = getattr(props, "name", "XPU")
            mem = getattr(props, "total_memory", 0) / (1 << 20)
            s += f"XPU:0 ({name}, {mem:.0f}MiB)\n"
        except Exception:
            s += "XPU:0\n"
        arg = "xpu"
    else:
