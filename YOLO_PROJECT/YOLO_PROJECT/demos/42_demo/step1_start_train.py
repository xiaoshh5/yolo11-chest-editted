...
@Author  ：肆十二（付费咨询QQ: 3045834499） 粉丝可享受99元调试服务
	# Pass the DirectML device name as a string (e.g. 'privateuseone:0')
	device_arg = str(dml_dev)
	print(f"Using DirectML device: {device_arg}")
'''
import time
...
data_yaml = os.path.join(project_root, 'chest_data.yaml')

# todo 请将A_my_data配置文件的路径修改为你本地的路径
# yolov11n模型训练：训练模型的数据为'A_my_data.yaml'，轮数为100，图片大小为640，设备为本地的GPU显卡，关闭多线程的加载，图像加载的批次大小为4，开启图片缓存
model = YOLO("yolo11n.yaml").load("yolo11n.pt") # load a pretrained model (recommended for training)
# Intel Arc A770 support: use XPU if available, else CPU.
try:
	# Check if Intel GPU is available
	if torch.xpu.is_available():
		device_arg = 'xpu'
		print(f"Using Intel GPU (XPU) device")
	else:
		device_arg = 'cpu'
		print("Intel GPU not available, falling back to CPU")
except Exception as e:
	device_arg = 'cpu'
	print(f"Error checking XPU availability: {e}, falling back to CPU")
...
results = model.train(
    ...
    device=device_arg,
    workers=0,
    batch=8,              # 增加批次大小到8（如果显存允许）
    cache=True,           # 启用缓存加速
    resume=False,
    amp=False,
    patience=20,          # 早停耐心值：20个epoch无改进则停止
...
# results = model.train(data='A_my_data.yaml', project="./", epochs=100, imgsz=640, device=[0, ], workers=0, batch=4, cache=True, resume=True)  # GPU 开始训练
...
# results = model.train(data='/root/autodl-tmp/code/yolo/yolo5_11/ultralytics/cfg/datasets/A_my_data.yaml', project="./runs", epochs=200, imgsz=640, device=[ ], workers=4, batch=16, cache=True, resume=True, amp=False)  # CPU 开始训练
# time.sleep(10) # 睡眠10s，主要是用于服务器多次训练的过程中使用
#
